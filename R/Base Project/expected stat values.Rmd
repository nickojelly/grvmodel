...
library("GGally")
library("readxl")
dataf = read_csv('C:/Users/Nick/Documents/GitHub/grvmodel/Python/DATA/model_all_price/cosmic-violet-428 - all_price_df.csv')
...
df_tbl_bin %>%  ggplot(aes(x=new_bin,y=cROI)) +geom_col() +facet_wrap(~ track)
...
df_tbl_bin <- df_tbl_bin %>% group_by(flat_date) %>% summarise(sum_prof = sum(`profit < 30`), sum_outlay = sum(`outlay < 30`), count = n(), win = sum(onehot_win))
df_tbl_bin$ROI <- df_tbl_bin$sum_prof/df_tbl_bin$sum_outlay
df_tbl_bin$csumc <- ave(df_tbl_bin$count, df_tbl_bin$track, FUN=cumsum)
df_tbl_bin$csumc <- df_tbl_bin$csumc/df_tbl_bin$count
df_tbl_bin$csump <- ave(df_tbl_bin$sum_prof, df_tbl_bin$track, FUN=cumsum)
