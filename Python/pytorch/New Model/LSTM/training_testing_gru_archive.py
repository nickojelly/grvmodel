    ...
    for epoch in trange(epochs):
        ...
        for i in range(num_batches):
            with torch.cuda.amp.autocast():
                ...
                output,relu = model(X2, p1=False)
                # relu_weight = F.softmax(relu.mean(dim=1))
                model.zero_grad(set_to_none=True)

                # print(f"{output=}\n{y=},{w=}")

                epoch_loss = criterion(output, y)*w# *len(batch_races)*relu_weight
    ...
    for epoch in trange(epochs):
        ...
        for i in range(num_batches):
            with torch.cuda.amp.autocast():
                ...
                epoch_loss = criterion(output, y)*w*len(batch_races)
