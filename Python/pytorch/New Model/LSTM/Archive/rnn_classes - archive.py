...
class Races:
    ...
    def create_dogs_test_split_date(self):
        for i in tqdm(self.dog_ids):
            dog = self.dogsDict[i]
            train = [dog.races[x] for x in dog.races.keys() if x in self.train_race_ids]
            test =  [dog.races[x] for x in dog.races.keys() if x in self.test_race_ids]
            if train:
                dog.train = train
    ...
    def reset_all_lstm_states(self):
        for race in self.racesDict.values():
            for dog in race.dogs:
                dog.lstmCellc = torch.rand(self.hidden_size).to('cuda:0')
                dog.lstmCellh = torch.rand(self.hidden_size).to('cuda:0')
    ...
    def create_hidden_states_dict_v2(self):
        ...
        for dog in self.test_dog_ids:
            self.hidden_states_dict_gru_v6[dog] = self.dogsDict[dog].hidden_test
    ...
    def normalize_inputs(self, normalizer=None):
        if not normalizer:
            print("generating normalizer across train")
    ...
    # def attach_races_to_dog_inputv3(self):
    #     for k,v in tqdm(self.dogsDict.items()):
    #         for race_id,dog_input in v.races.items():
    #             dog_input.race = self.racesDict[race_id]
    #             dog_input.full_input = torch.cat([dog_input.race.race_dist,dog_input.race.race_track,dog_input.stats], dim = 0)
    ...
    def reset_hidden(self):
        for dog in self.dogsDict.values():
            dog.hidden = torch.zeros(2,self.hidden_size).to('cuda:0')
