...
def clean_data(df):
    ...
    df['pred_prob2'] = 1 / df['pred_prices2']
    ...
    df['bet_relu'] = df['bet_amount'] * df['flat_relu']
    ...
    df['bet_relu<30'] = np.where(df['prices'] < 20, df['bet_amount'] * df['flat_relu'], 0)
...
def validate_model_v3(model:GRUNetv3,raceDB:Races,criterion=None, batch_size=None,epoch=10,config=None,device='cuda:0'):
    ...
    with torch.no_grad():
        ...
        flat_date_df = all_price_df[['flat_date','outlay < 30','profit < 30','bet_amount','profit','profit_relu','profit_relu<30']].groupby('flat_date').sum().cumsum().reset_index()
