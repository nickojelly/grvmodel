...
def generate_prediction_dataframe_v2(prev_results_file:str, prediction_df, ohe='encoder_new'):
    ...
    prediction_df = dd.from_pandas(prediction_df)

    form = dd.concat([form, prediction_df])
    ...
    # form_stats = generate_dog_stats(form_stats, dog_form,rolling_window=10)
    ...
    form = form[['DogId', 'DogName', 'RaceId','race_num','RaceGrade', 'dateF', 'race_time','tracksOnehot','Track','State','track_code','dist_x', 'BSP','RunTime','prev_race','prev_race_date','Place','margin','stats']]
    form.columns=['dogid', 'dog_name', 'raceid','race_num','race_grade', 'date','race_time', 'trackOHE','track_name', 'state', 'track_code','dist','bfSP','runtime','prev_race', 'prev_race_date', 'place', 'margin','stats']
...
def generate_results_df_v2(previous_results_file, new_FT : pd.DataFrame ,new_FT_race_data : pd.DataFrame, betfair_sp_file :str, split_dist:str, mode="all", prev_df=None, ohe='encoder_new'):
    ohe = pickle.load(open(ohe, 'rb'))

    # previous_results = pd.read_pickle(previous_results_file)
    # dog_results = pd.concat([previous_results, dog_results])
    ...
    form = form[['DogId', 'DogName', 'RaceId', 'RaceGrade', 'dateF', 'tracksOnehot','Track','State','track_code','dist_x', 'BSP','RunTime','prev_race','prev_race_date','Place','margin','stats']]
    form.columns=['dogid', 'dog_name', 'raceid', 'race_grade', 'date', 'trackOHE','track_name', 'state', 'track_code','dist','bfSP','runtime','prev_race', 'prev_race_date', 'place', 'margin','stats']
