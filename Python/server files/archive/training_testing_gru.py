...
def train_regular_v3(model:GRUNetv3,raceDB:Races, criterion, optimizer,scheduler, config=None,update=False):
    ...
    for epoch in trange(epochs):
        ...
        if (epoch)%20==0:
            validate_model_v3(model,raceDB, criterion=criterion, epoch=epoch, device='cpu')
        if (epoch)%100==0:
...
def validate_model_v3(model:GRUNetv3,raceDB:Races,criterion=None, batch_size=None,epoch=10,config=None,device='cuda:0'):
    ...
    with torch.no_grad():
        ...
        all_price_df.flat_date = pd.to_datetime(all_price_df.flat_date).dt.date
        all_price_df.race_num = pd.to_numeric(all_price_df.race_num)
        all_price_df = all_price_df[~all_price_df['track'].isin(['Dapto','Nowra','Taree','Wagga Wagga'])]
        ...
        all_price_df['bet_relu<30'] = all_price_df.apply(lambda x: x['bet_relu'] if x['prices']<20 else 0, axis = 1)
        ...
        flat_track_df['ROI < 30 relu'] = flat_track_df.apply(lambda x: x['profit_relu']/x['bet_relu'] if x['bet_relu']>0 else 0,axis =1).tolist()
        ...
        flat_date_df = all_price_df[['flat_date','outlay < 30','profit < 30','bet amount','profit','profit_relu<30','bet_relu<30','flat_loss', 'onehot_win', 'flat_correct']].groupby('flat_date').sum().cumsum().reset_index()
        flat_date_df['roi<30_relu'] = flat_date_df['profit_relu<30']/flat_date_df['bet_relu<30']
        flat_date_df['loss'] = flat_date_df['flat_loss']/flat_date_df['onehot_win']
        flat_date_df['accuracy'] = flat_date_df['flat_correct']/flat_date_df['onehot_win']
        ...
        flat_date_df = all_price_df[['flat_date','outlay < 30','profit < 30','bet amount','profit','profit_relu<30','bet_relu<30','flat_loss', 'onehot_win', 'flat_correct']].groupby('flat_date').sum().rolling(window=30).sum().reset_index()
        flat_date_df['loss'] = flat_date_df['flat_loss']/flat_date_df['onehot_win']
        flat_date_df['accuracy'] = flat_date_df['flat_correct']/flat_date_df['onehot_win']
        flat_date_df['roi<30_relu'] = flat_date_df['profit_relu<30']/flat_date_df['bet_relu<30']
        flat_date_df['roi<30'] = flat_date_df['profit < 30']/flat_date_df['outlay < 30']
        ...
        flat_date_df = all_price_df[['flat_date','outlay < 30','profit < 30','bet amount','profit','profit_relu<30','bet_relu<30','flat_loss', 'onehot_win', 'flat_correct']].groupby('flat_date').sum().reset_index()
        flat_date_df['loss'] = flat_date_df['flat_loss']/flat_date_df['onehot_win']
        flat_date_df['accuracy'] = flat_date_df['flat_correct']/flat_date_df['onehot_win']
        flat_date_df['roi<30_relu'] = flat_date_df['profit_relu<30']/flat_date_df['bet_relu<30']
        flat_date_df['roi<30'] = flat_date_df['profit < 30']/flat_date_df['outlay < 30']
        flat_date_df_simple_wandb = wandb.Table(dataframe=flat_date_df.reset_index())

        flat_date_df = all_price_df[['flat_date','outlay < 30','profit < 30','bet amount','profit','profit_relu<30','bet_relu<30','flat_loss', 'onehot_win', 'flat_correct']].groupby('flat_date').sum().rolling(window=30).sum().reset_index()

        flat_track_wandb = wandb.Table(dataframe=flat_track_df.reset_index())
        try:
            wandb.log({'flat_track':flat_track_wandb}, step=epoch)
            wandb.log({'flat_date':flat_date_df_wandb})
            wandb.log({'flat_date_sum':flat_date_df_sum_wandb})
            wandb.log({'flat_date_sum_simple':flat_date_df_simple_wandb})
        except Exception as e:
            print(e)
        ...
        # try:
        #     wandb.log({'flat_track3':wandb.Table(dataframe=flat_track_df)})
        # except Exception as e:
        #     print(e)
        if epoch%100==0:
