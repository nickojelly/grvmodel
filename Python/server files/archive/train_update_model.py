...
def load_db(states,hidden_size=128):
    ...
    raceDB = build_dataset(pred_db, hidden_size, state_filter=states, margin_type='boosted_sftmin',v6=True)
    today = datetime.datetime.today().strftime('%Y-%m-%d')
...
def model_pipeline(my_dataset,config=None,prev_model=None, sweep=True, model_state_dict=None, prev_model_file=None, prev_model_version='450', v6=False):
	...
	with wandb.init(project="NEW GRU - updates", config=config):	
		...
		print(raceDB.batches.keys())
		# print([len(x) for x in raceDB.batches.values() if ~isinstance(x,int)])
		for k,v in raceDB.batches.items():
			print(k)
			try:
				print(len(v))
			except:
		...
		training_testing_gru.train_regular_v3(model, raceDB, criterion,optimizer,scheduler='na',config=config,update=True)
...
if __name__ == '__main__':
	...
	if v6:
		...
		raceDB = build_dataset("/root/grv_model/db_update/DATA/gru_inputs_new_simple_test.fth", 128, state_filter=states, margin_type='boosted_sftmin',v6=True,  device='cpu')
