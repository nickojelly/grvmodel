...
def generate_prediction_dataframe_v2(prev_results_file:str, prediction_df, ohe='encoder_new',simple=False,v6=False):
    ...
    form = form[['DogId', 'DogName', 'RaceId','race_num','RaceGrade', 'date', 'race_time','tracksOnehot','Track','tab_track','State','track_code','dist_x', 'StartPrice','BSP','RunTime','prev_race','prev_race_date','prev_race_track','Place','margin','box','stats']]
    form.columns=['dogid', 'dog_name', 'raceid','race_num','race_grade','date','race_time', 'trackOHE','track_name','tab_track', 'state', 'track_code','dist','StartPrice','bfSP','runtime','prev_race', 'prev_race_date','prev_race_track', 'place', 'margin','box','stats']
...
def generate_results_df_v2(previous_results_file, new_FT : pd.DataFrame ,new_FT_race_data : pd.DataFrame, betfair_sp_file :str, split_dist:str, mode="all", prev_df=None, ohe='encoder_new', simple=False,normalize=False,v6=False):
    ...
    # form = form[form['State'].isin(['NZ'])].reset_index()
    ...
    form_stats = form_stats[form_stats['dateF']>date]
    form_stats.reset_index(inplace=True)
    ...
    form = form[['DogId', 'DogName', 'RaceId', 'RaceGrade', 'dateF','race_time','RaceNum','tracksOnehot','Track','State','track_code','dist_x', 'StartPrice','BSP','RunTime','prev_race','prev_race_date','prev_race_track','prev_race_state','Place','margin','box','stats']]
    form.columns=['dogid', 'dog_name', 'raceid', 'race_grade', 'date','race_time','race_num','trackOHE','track_name', 'state', 'track_code','dist', 'StartPrice','bfSP','runtime','prev_race', 'prev_race_date','prev_race_track','prev_race_state', 'place', 'margin','box','stats']
    print('fin')
    return (form,stats_cols)
