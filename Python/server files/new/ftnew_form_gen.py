...
def get_betfair_data():
    ...
    for x in box_change_df['string'].tolist():
        print(x)
        print(regexp_tokenize(x, pattern1)[0])
    ...
    box_change_df['RaceBox'] = pd.to_numeric(box_change_df['string'].apply(lambda x: regexp_tokenize(x, pattern2)[0] if  regexp_tokenize(x, pattern2) else "NA"))
