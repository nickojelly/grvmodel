        ...
        if market.seconds_to_start < 30 and market_book.inplay == False and track in tracks:
            ...
            for runner_cata, runner in zip(market.market_catalogue.runners, market_book.runners):
                ...
                if runner_cata.selection_id == runner.selection_id:
                    ...
                    model_relu = todays_data.loc[dog_name,track,race_number]['relu_sum'].item()
                    ...
                    if runner.status == "ACTIVE" and runner.ex.available_to_lay[0]['price'] > model_price and model_price>1 and runner.ex.available_to_lay[0]['price'] < 20 :
                        #bf_price = runner.ex.available_to_back[0]['price']
                        implied_prob = 1/runner.ex.available_to_lay[0]['price'] 
                        bet_amount = round((model_prob-implied_prob)*100*model_relu*0.75,2)
                        logging.info(f"BF avail price: lay: {runner.ex.available_to_lay[0]['price']} back: {runner.ex.available_to_back[0]['price']}")
                        logging.info(f'bet_amount: {bet_amount}')
                        logging.info(f'relu : {model_relu}')
                        ...
                        order = trade.create_order(
                            side="BACK", order_type=LimitOrder(price=runner.ex.available_to_lay[0]['price'], size=bet_amount, persistence_type='MARKET_ON_CLOSE')
                        )
...
if __name__=="__main__":
    ...
    todays_data = pd.read_pickle(f'./inputs/output {today}_v6.npy')
    model_name = todays_data['Model Name'].iloc[1]
    ...
    todays_data.replace('Palmerston North','Manawatu',inplace = True)
    todays_data.replace('Christchurch','Addington',inplace = True)
    todays_data.replace('Auckland','Manukau',inplace = True)
    todays_data.replace('Waikato','Manukau',inplace = True)
    todays_data.replace('Wagga Wagga','Wagga',inplace = True)
    todays_data.replace('Southland', 'Ascot Park', inplace = True)
    todays_data.replace('Richmond Straight','Richmond',inplace = True)
    todays_data.replace('Wanganui','Hatrick',inplace = True)
    todays_data = todays_data[~todays_data['track'].isin(['Nowra','Dapto','Taree'])]
    ...
    greyhounds_strategy = FlatBetting(
        market_filter=streaming_market_filter(
            ...
            country_codes=["AU","NZ"], # Australian markets
