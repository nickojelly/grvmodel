...
def get_fasttrack_data():
    ...
    fasttrack_df.DogName = fasttrack_df.DogName.apply(lambda x: x.replace("'", "").replace(".", "").replace("Res", "").strip() if x else "")
...
def get_betfair_data():
    ...
    for i in results_list:
        ...
        if i.description.clarifications:
            ...
            box_change_list.append((track, race_num,i.description.clarifications.replace("<br> Dog","<br>Dog").replace("<Br>","<br>")))
    ...
    box_change_df = box_change_df[~(box_change_df['string']=='  ')]
    box_change_df = box_change_df[~(box_change_df['string']=='')]
    box_change_df = box_change_df[box_change_df['string'].str.contains('[1-9]',regex=True)]
    for i in box_change_df['string'].tolist():
        print(i)
        print(regexp_tokenize(i,pattern1))
    ...
    box_change_df['RaceBox'] = pd.to_numeric(box_change_df['string'].apply(lambda x: regexp_tokenize(x, pattern2)[0] if regexp_tokenize(x, pattern2) else 0))
...
if __name__ == "__main__":
    ...
    logging.info(f"Creating new inputs for model predictions")
    ...
    # races_bf_df.replace('Ascot Park', 'Southland',inplace=True)

    # if 'Richmond Straight' in tracks_FT:
    #     races_bf_df.replace('Richmond','Richmond Straight',inplace = True)
    #     print("replacing richmond")
    # if 'Auckland' in tracks_FT:
    #     races_bf_df.replace('Manukau','Auckland',inplace = True)
    #     print('replacing Auckland')
    # if 'Waikato' in tracks_FT:
    #     races_bf_df.replace('Manukau','Waikato',inplace=True)
    #     print('replacing Waikato')
    ...
    fasttrack_df = fasttrack_df.merge(races_bf_df, on=['tab_track', 'RaceNum', 'RaceBox'])
    merged = fasttrack_df.merge(how='left', right=box_change_df.drop(columns='tab_track'), right_on=['runner_name', 'RaceNum'], left_on=['DogName', 'RaceNum'])
    print(f'{merged=}')
    ...
    new_form = merged[merged['Box']<=8][['Track','tab_track','State','DogId','DogName','DogGrade', 'RaceId','Distance', 'Date', 'Box','race_time','RaceNum']].rename(columns={ 'RaceNum':'race_num'})
    ...
    x.reset_index(drop=True).to_feather(f"//root/grv_model/model_predictions/prediction_inputs/testing new outs simple {today}{v6}.fth")
    x.reset_index(drop=True).to_excel(f"//root/grv_model/model_predictions/prediction_inputs/testing new outs simple {today}{v6}.xlsx")
