...
class Races:
    ...
    def create_batches(self, batch_days = 365):
        ...
        packed_x = ""#[pack_sequence([torch.stack(n,0) for n in [[z.full_input for z in inner] for inner in x]], enforce_sorted=False).to(DEVICE) for x in train_dog_input]
        ...
        packed_y = ""#pack_sequence([torch.stack(n,0) for n in [[z.full_input.to(DEVICE) for z in inner] for inner in [x for x in raceDB.get_dog_test(test_idx)]]], enforce_sorted=False)
    ...
    def reset_hidden(self, num_layers=2,hidden_size=None,device=DEVICE):
        if hidden_size==None:
