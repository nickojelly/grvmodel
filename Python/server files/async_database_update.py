...
async def update_results_data():
    seckey = SECKEY
    greys = ft.Fasttrack(seckey)
    today = (datetime.today()+timedelta(days=1)).strftime('%Y-%m-%d')
    lastweek = (datetime.today()-timedelta(days=7)).strftime('%Y-%m-%d')
    race_details, dog_results = await greys.getRaceResults('2019-12-01', dt_end=today)
...
async def update_basic_form_data():
    seckey = SECKEY
    greys = ft.Fasttrack(seckey)
    today = (datetime.today()+timedelta(days=1)).strftime('%Y-%m-%d')
    lastweek = (datetime.today()-timedelta(days=7)).strftime('%Y-%m-%d')

    race_details, dog_results = await greys.getBasicFormat('2019-12-01', dt_end=today)
    ...
    all_races.reset_index(drop=True).to_feather(r'./DATA/all basic form merged.fth')
    return all_races
...
if __name__ == "__main__":
    ...
    dog_results = asyncio.run(update_results_data())
    basic_form = asyncio.run(update_basic_form_data())

    dog_results = dog_results.merge(basic_form, how='left', on=['RaceId', 'DogId'])
    dog_results = dog_results.dropna(subset=['track_code'])
    dog_results.reset_index(drop=True).to_feather(r'./DATA/ft_races.fth')
    ...
    x,stats_cols = featurecreations.generate_results_df_v2(prev_full_details, dog_results, 'na', betfair_sp_file, split_dist_file,mode="OO", prev_df="AHHHHH", ohe='encoder_new', simple=simple, normalize=normalize)
    ...
    x.to_feather(f"./DATA/gru_inputs_new{simp}{norm}_test.fth")
