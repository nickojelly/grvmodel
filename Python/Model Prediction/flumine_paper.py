...
class LiveLoggingControl(LoggingControl):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._setup()
        self.file_name = "orders_hta_4-papers.csv"
    ...
    def _setup(self):
        if os.path.exists(self.file_name):
            ...
            with open(self.file_name, "w") as m:
...
if __name__=="__main__":
    ...
    client = clients.BetfairClient(trading, interactive_login=True,paper_trade=True)
    ...
    logging.basicConfig(filename = f'./logs/fluminelog {today} - papers.log', level=logging.INFO, format='%(asctime)s:%(levelname)s:%(message)s')
