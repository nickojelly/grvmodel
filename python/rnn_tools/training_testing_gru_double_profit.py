    ...
    for epoch in trange(epochs):
        ...
        for i in range(num_batches):
            with torch.cuda.amp.autocast():
                ...
                X3 = torch.cat([output,output_p,y_p],dim=1)
                Y3 = torch.stack([x.one_hot_class for x in race])
                P3 = torch.stack([x.prices_tensor for x in race])

                output_bet = model(X3,p1=False,p2=True)+0.001

                # print(output_bet)
                # print(P3)

                output_profit = (output_bet*Y3*P3)*0.95-output_bet

                output_profit_loss = (-(output_profit)/output_bet).mean(dim=1)

                # print(f"{epoch_loss.shape=}")
                # print(f"{epoch_loss_p.shape=}")
                # print(f"{output_profit_loss.shape=}")
                # print(f"{output_profit_loss.mean(dim=1).shape=}")

                # print(epoch_loss)
            t6 = time.perf_counter()
            if not update:
                (epoch_loss+epoch_loss_p+output_profit_loss).mean().backward()
                optimizer.step()
                wandb.log({"loss_1": torch.mean(epoch_loss).item(), 'epoch':epoch, "output_profit":output_profit.sum(),"output_profit_loss":output_profit_loss.mean()}, step = example_ct)
...
def test_model_v3(model:GRUNetv3,raceDB:Races,criterion=None, batch_size=None,epoch=10,config=None,device='cuda:0'):
    ...
    with torch.no_grad():
        ...
        output,hidden = model(X,h=hidden_in,p2=False) # Shape List[Tensor[Dog]]
        ...
        output,relu,output_p= model(Xt, p1=False,p2=False)
        ...
        stats_dict = {"accuracy": correct/len_test,
                    ...
                    'epoch':epoch,
                    "test_output_profit":output_profit.mean(),"test_output_profit_loss":output_profit_loss.mean()
                    }
...
def validate_model_v3(model:GRUNetv3,raceDB:Races,criterion=None, batch_size=None,epoch=10,config=None,device='cuda:0'):
    ...
    with torch.no_grad():
        ...
        races['pred_prob2'] = softmax_preds2.tolist()
        races['prices'] = [x.prices for x in test_races]
        races['imp_prob'] = [x.implied_prob.cpu().tolist()  for x in test_races]
        races['pred_price'] = (1/softmax_preds).tolist()
        races['pred_price1'] = (1/softmax_preds1).tolist()
        races['pred_price2'] = (1/softmax_preds2).tolist()
        # races['pred_prob'] = [x.tolist() for x in races['pred_prob']]
        ...
        stats_dict = {"val_accuracy": correct/len(raceDB.val_race_ids),
                    ...
                    'epoch':epoch,
                    "val_output_profit":output_profit.mean(),"val_output_profit_loss":output_profit_loss.mean()
                    }
