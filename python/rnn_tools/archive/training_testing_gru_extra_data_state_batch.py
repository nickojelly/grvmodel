...
def train_double_v3(model:GRUNetv3,raceDB:Races, criterion, optimizer,scheduler, config=None,update=False):
    ...
    for epoch in trange(epochs):
        ...
        for i in range(num_batches):
            ...
            [setattr(obj, 'hidden', val) for obj, val in zip(dogs,hidden.detach())]
            batch_races = raceDB.batches['new_batch_races'][i]
            state_batch_loss = torch.tensor(0.0,requires_grad=True)
            for i,state_batch in enumerate(batch_races):
                print(f'state_batch {raceDB.states[i]}, len: {len(state_batch)}')
                example_ct+=len(state_batch)

                [setattr(race, 'hidden_in', torch.cat([race.race_dist]+[race.race_track]+[d.hidden_out for d in race.dogs])) for race in state_batch]
                ...
                # [setattr(race, 'loss', loss.mean().detach()) for race,loss in zip(state_batch,criterion(output, y))]

                p2 = time.perf_counter()
                # print(f"Second 2nd Pass: {p2-p1}")
                p1 = time.perf_counter()
                state_batch_loss = state_batch_loss + (epoch_loss.mean()+epoch_loss_p.mean()).mean()

                raceDB.dogsDict['nullDog'].input.hidden_out = (-torch.ones(256+64)).to('cuda:0')
            optimizer.zero_grad()
            (state_batch_loss.mean()).backward()
            optimizer.step()
            model.zero_grad()
            raceDB.detach_hidden(dogs)

            t6 = time.perf_counter()
            epoch_loss = state_batch_loss
