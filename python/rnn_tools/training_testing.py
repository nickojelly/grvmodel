...
def validate_model(model:GRUNet,raceDB:Races,criterion, batch_size, example_ct, epoch_loss, batch_ct,epoch,config):
    ...
    with torch.no_grad():
        for i in trange(0,len_test,batch_size, leave=False):
            ...
            price_dict['loss'].extend(loss_tensor.tolist())
...
def train_super_batch_L1(model, raceDB:Races, criterion, optimizer,scheduler, config=None, loss_fun='CEL'):
    torch.autograd.set_detect_anomaly(True)
    ...
    for epoch in trange(epochs):
        ...
        for i,batch in enumerate(batch_indexes):
            ...
            y = torch.stack([x.classes for x in race])
            w = torch.stack([x.margin_weights for x in race])
            _, actual = torch.max(y.data, 1)
            output = model(X)
        ...
        #if epoch%3==0:
        acc, loss_val = validate_model_L1(model,raceDB,criterion, 8, example_ct, epoch_loss, batch_ct)
...
def train_super_batch_KL(model, raceDB:Races, criterion, optimizer,scheduler, config=None, loss_fun='CEL'):
    torch.autograd.set_detect_anomaly(True)
...
def train_super_batch(model, raceDB:Races, criterion, optimizer,scheduler, config=None, loss_fun='CEL'):
    torch.autograd.set_detect_anomaly(True)
...
def train_double_loss_regular(model, raceDB:Races, criterion, optimizer,scheduler, config=None, crit2=None):
    # torch.autograd.set_detect_anomaly(True)
    ...
    for epoch in trange(epochs):
        ...
        for i in trange(0,len_train-batch_size,batch_size):
            ...
            loss = criterion(output, y)*w
            loss2 = crit2(output,y)*w*3
            if setup_loss:
                epoch_loss = loss+loss2
                setup_loss=0
