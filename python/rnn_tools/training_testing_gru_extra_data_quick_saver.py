...
from rnn_tools.model_saver import model_saver_wandb
from rnn_tools.model_saver import model_saver_second
...
def train_regular_v3(model:GRUNetv3,raceDB:Races, criterion, optimizer,scheduler, config=None,update=False):
    ...
    for epoch in trange(epochs):
        ...
        if (epoch)%100==0:
            ...
            model_saver_wandb(model, optimizer, epoch, 0.1, raceDB.hidden_states_dict_gru_v6, raceDB.train_hidden_dict,path='second_models_cache')
            if update:
                break
        if not update:
            #print('reset hidden')
...
def train_quick_pass(model:GRUNetv3,raceDB,config):
    ...
    for i in range(num_batches):
        ...
        y = torch.stack([x.classes for x in race])
        [setattr(race, 'hidden_in', torch.cat([race.race_dist]+[race.race_track]+[d.hidden_out for d in race.dogs])) for race in batch_races]
        X2 = torch.stack([r.hidden_in for r in race])
...
def train_double_v3(model:GRUNetv3,raceDB:Races, criterion, optimizer,scheduler, config=None,update=False):
    ...
    if (epoch)%10==0:
        ...
        raceDB.create_hidden_states_dict_v2()
        raceDB.create_hidden_in_dict()
        model_saver_second(model, optimizer, epoch, 0.1, raceDB.hidden_states_dict_gru_v6, raceDB.train_hidden_dict,raceDB,path='second_models_cache')
