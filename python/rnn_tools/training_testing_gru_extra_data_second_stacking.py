...
def simple_profit(simple_model,prices,classes,output,output_p):
...
def train_double_v3(model:GRUNetv3_extra_quick_stack,raceDB:Races, criterion, optimizer,scheduler, config=None,update=False):
    ...
    load_prev_stack_model = True
    if load_prev_stack_model:
        prev_model_file = 'VIC-zesty-music-418'
        model_name = prev_model_file
        prev_model_version = 15
        print(f"Loading base model model {prev_model_file}, version {prev_model_version}")
        ...
        for i,m in enumerate(model.model_list):
            ...
            m.load_state_dict(model_data['model_state_dict'],strict=False)
    ...
    for epoch in trange(epochs):
        ...
        for i,batch_races in enumerate(data_loader):
            with torch.cuda.amp.autocast():
                ...
                simple_optim = model.optim_list[i]
                race = batch_races
            ...
            if not update:
                ...
                print(f"{(epoch_loss_p+epoch_loss).mean()=}")
                simple_optim.step()
                simple_model.zero_grad()
                wandb.log({f"train_loss_{i}": torch.mean(epoch_loss).item(), 'epoch':epoch,f'train_profit_loss_{i}':profit})
        ...
        for i,batch_races in enumerate(data_loader):
            with torch.cuda.amp.autocast():
                ...
                over_batch_profits.append(profit)
                profit = profit.mean()
                ...
                reg_losses.append(epoch_loss+epoch_loss_p)
                # [setattr(race, 'loss', loss.mean().detach()) for race,loss in zip(batch_races,criterion(output, y))]
        ...
        if not update:
            optimizer.zero_grad()
            torch.cat(reg_losses).mean().backward()
            optimizer.step()
            # profit.backward()
            # profit_optim.step()
            model.zero_grad()
