...
def train_double_v3(model:GRUNetv3_extra_fast_inf,raceDB:Races, criterion, optimizer,scheduler, config=None,update=False):
    ...
    # profit_model = GRUNetv3_profit_stacking(raceDB).to('cuda:0')
    profit_optim = optim.Adam(profit_model.parameters(), lr=0.001,maximize=True)

    raceDB.profit_model = profit_model
    full_optim = optim.Adam(list(model.parameters())+list(profit_model.parameters()), lr=0.001, maximize=True)
    prev_model_file='lucky-plant-226'
    prev_model_version=3280
    ...
    for epoch in trange(epochs):
        ...
        for i in range(num_batches):
            with torch.cuda.amp.autocast():
                ...
                profit_tensor = simple_profit(profit_model,p,y,output,output_p)*profit_weight
                profit_by_race = profit_tensor.nan_to_num(0,0,0).sum(dim=1).detach()

                #split profitability using relu into -ve and +ve
                ...
                pos_profit = relu(profit_by_race)
                neg_profit = relu(-profit_by_race)
                profit_weight = neg_profit+0.5
