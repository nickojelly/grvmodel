...
def train_double_v3(model:GRUNetv3,raceDB:Races, criterion, optimizer,scheduler, config=None,update=False):
    ...
    profit_optim = optim.Adam(list(profit_model.parameters())+list(model.parameters()), lr=0.001,maximize=True)
    ...
    for epoch in trange(epochs):
        ...
        for i in range(num_batches):
            ...
            if not update:
                ...
                # optimizer.step()
                model.zero_grad(set_to_none=True)
                profit_model.zero_grad(set_to_none=True)    
        ...
        if (epoch)%10==0:
            ...
            # if (test_stats['ROI < 30'] > test_max_roi or val_stats['val_ROI < 30'] > val_max_roi) or val_stats['val_loss_val'] < val_loss_min:
                ...
                model_saver_wandb(model, optimizer, epoch+1, test_max_roi, raceDB.hidden_states_dict_gru_v6, raceDB.train_hidden_dict, model_name="long nsw new  22000 RUN")
            t9 = time.perf_counter()
        torch.cuda.empty_cache()
