...
class Races:
    ...
    def create_test_split_date(self, start_date):
        ...
        self.test_race_ids = []
        self.val_race_ids = []
        val_race_date = datetime.datetime.strptime("2023-06-01", "%Y-%m-%d").date()
    ...
    def get_dog_val(self, idx):
        ...
        races = [self.val_dogs[x].test for x in dog_batch_id] 
        return races 
    ...
    def create_batches(self,end_date="2023-06-01", batch_days = 365):
        ...
        packed_v_basic = pack_sequence([torch.stack(n,0) for n in [[z.stats for z in inner] for inner in [x for x in self.get_dog_val(test_idx)]]], enforce_sorted=False)
